...
from .const import ATTRIBUTION, DOMAIN, DEVICE_AUTHOR, DEVICE_VERSION
...
async def async_setup_entry(
    ...
    for device_id, device in coordinator.devices.items():
        ...
        disabled_capabilities = main_status.get("custom.disabledCapabilities", {}).get("disabledCapabilities", {}).get("value", [])
        
        if "samsungce.powerCool" in capability_ids and "samsungce.powerCool" not in disabled_capabilities:
            _LOGGER.info("Creating Power Cool switch for device %s", device.get("label", device_id))
            entities.append(SmartThingsPowerCoolSwitch(coordinator, api, device_id))
        elif "samsungce.powerCool" in capability_ids:
            _LOGGER.debug("Skipping Power Cool switch for device %s - disabled", device.get("label", device_id))
        
        if "samsungce.powerFreeze" in capability_ids and "samsungce.powerFreeze" not in disabled_capabilities:
            _LOGGER.info("Creating Power Freeze switch for device %s", device.get("label", device_id))
            entities.append(SmartThingsPowerFreezeSwitch(coordinator, api, device_id))
        elif "samsungce.powerFreeze" in capability_ids:
            _LOGGER.debug("Skipping Power Freeze switch for device %s - disabled", device.get("label", device_id))
    ...
    def device_info(self) -> DeviceInfo:
        ...
        for component_id, component_status in status.items():
            if "samsungce.softwareVersion" in component_status:
                versions = component_status["samsungce.softwareVersion"].get("versions", {}).get("value", [])
        ...
        for component_id, component_status in status.items():
            if "samsungce.softwareUpdate" in component_status:
                model = component_status["samsungce.softwareUpdate"].get("otnDUID", {}).get("value")
                if model:
                    break
    ...
    def is_on(self) -> Optional[bool]:
        ...
        for component_id, component_status in status.items():
            if "samsungce.powerCool" in component_status:
                activated = component_status["samsungce.powerCool"].get("activated", {}).get("value")
...
class SmartThingsPowerFreezeSwitch(CoordinatorEntity, SwitchEntity):
    ...
    def is_on(self) -> Optional[bool]:
        ...
        for component_id, component_status in status.items():
            if "samsungce.powerFreeze" in component_status:
                activated = component_status["samsungce.powerFreeze"].get("activated", {}).get("value")
                # None means capability is disabled on this device - this shouldn't happen if setup is correct
                # but handle it gracefully
                if activated is None:
                    return False
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            _LOGGER.error("Failed to turn off power freeze %s: %s", self._device_id, err)
